...
### Hash collision（`% 256` 造成的桶碰撞）

不同 token 經 `idx = int(digest, 16) % 256` 後**可能落到同一格**，這是 feature hashing 的預期取捨，不是 bug：

```
"ai"     → idx = 42  → vec[42] += 1
"react"  → idx = 42  → vec[42] += 1   ← 不同詞，同一格，訊息混在一起
```
...
| 類型 | 說明 | 嚴重性 |
|------|------|--------|
| **Bucket collision** | 不同 token 對 `% 256` 取餘後索引相同 | **一定會發生**（桶只有 256 個） |
| **MD5 collision** | 兩個不同字串算出同一個 MD5 | 機率極低，本專案可忽略 |

**對本專案的影響**：

| 用途 | 影響 |
|------|------|
| 記憶檢索（短話題） | token 少，碰撞相對少；`"AI agents"` vs `"React 19"` 多半仍能分開 |
| 爬蟲去重（標題+摘要） | token 多，碰撞較常見，可能誤判相似或不相似 |

維度只有 256，碰撞比 768／1536 維的真實 embedding 更頻繁，語意區分力也較弱；但在「比字詞重疊 + 可調門檻」的前提下，練習流程仍夠用。

**若碰撞造成困擾，可考慮**（由輕到重）：

1. 加大 `EMBED_DIM`（例如 512、1024）→ 每桶平均更少 token
2. 使用 signed hash（`+1` / `-1`）→ 減輕不同詞堆在同一格的偏差
3. 換成真正的 embedding model → 語意級檢索，幾乎沒有這種同桶混詞問題
...
5. **256 維 feature hashing 有桶碰撞**：不同 token 可能映射到同一格（見上方「Hash collision」）；長文或近義詞場景精度有限。
6. **記憶不入 git**：每人本機累積不同；要分享可手動複製 `topic_memory.json`。
