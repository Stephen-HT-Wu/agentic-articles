...
- **開放文字之後才發現的 cover-crop 裁切 bug**：`OPENAI_IMAGE_SIZE="1024x1536"`（2:3=0.667）跟畫布 9:16（0.5625）長寬比差很多，`_cover_crop` 會裁掉約 100px/邊（以 1024 寬計）——`gpt-image-1-mini` 時代因為畫面上沒有文字，裁掉邊緣看不太出來；換 `gpt-image-2` 開始畫資訊圖表後，實測真的看到版面邊緣的文字欄位被裁掉一半。改成 `1024x1792`（0.571，非常接近 9:16）後 crop 只剩約 8px/邊，重新測一張刻意在左右兩側放文字欄位的流程圖，完全沒有被裁切
