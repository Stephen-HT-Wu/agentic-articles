...
`.env` 需要：
```
OPENAI_API_KEY=...        # 場景圖片生成（gpt-image-1-mini）
...
- 每個 scene 呼叫 `gpt-image-1-mini`（`size=1024x1536`、`quality=low`），存下 `b64_json`，用 `usage.input_tokens`/`output_tokens` 精確算成本（不用估）
...
- **真的帶場景圖片的完整版還沒實測**：OpenAI 帳號 billing hard limit 還沒調高，目前實測數字裡的影片全部場景都是退回背景圖，還沒驗證「不同段落畫面真的不一樣」在真實圖片下的效果（已用假的三色測試圖驗證過 xfade 串接跟畫面確實切換的機制本身沒問題）
